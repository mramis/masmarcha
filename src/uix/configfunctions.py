...
class ConfigOption2(ConfigWidget):
    ...
    def change_option(self):
        try:
            value = int(self.ids.showvalue.text)
            self.current_value = value if value > 0 else self.minvalue
        except ValueError:
    ...
    def down_change_option(self):
        try:
            value = self.current_value - self.interval
            self.current_value = value if value > 0 else self.minvalue
        except RecursionError as error:
            logging.error(error)
    ...
    def validate(self, value):
        ...
        if self.lower_widget and value <= self.lower_widget.current_value:
            self.up_change_option()
...
class ConfigOption3(ConfigOption2):
    ...
    def change_option(self):
        try:
            value = float(self.ids.showvalue.text)
            self.current_value = value if value > 0 else self.minvalue
