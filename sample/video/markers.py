...
class markerscollections(object):
    ...
    def interpolate(self):
        '''Con una muestra diseñada con funciones conocidas funciona como es
        es de esperarse. Se debe documentar este diseño.'''
...
# si es un cuadro falso con contenido a interpolar, entonces se agrega pero
# tambien aumenta en una unidad la variable step
                    step += 1
                    interpolated_markers.introduce(marker_array)
                elif marker_array.mean() == -1.0:
# si es un cuadro vacio, donde no existen marcadores entonces se agrega pero
# tambien se incrementan las variables step y jump. Si jump llega a un valor
# de 5 entonces se reinicia la variable step, porque significaria que no hay
# persona marchando en el video.
                    jump += 1
                    step += 1
                    interpolated_markers.introduce(marker_array)
                    if jump >= 5:
...
# cuando el cuadro tiene marcadores, y ademas la variable step es distinta de
# cero se deben borrar los ultimos "step" cuadros, interpolar, y agregar los
# los cuadros interpolados a la coleccion, setear la variable step en valor
# cero para que en el próximo cuadro, probablemente exitoso, el bucle no entre
# en el proceso de interpolación. Además se modifican las variables
# estadísticas
# Si la variable step es igual a cero, entonces se agrega el cuadro
# y el cuadro actual que contiene marcadores, se convierte en el ultimo capaz
# de interpolar.
                    jump = 0
                    if step != 0:
