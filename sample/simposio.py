...
directorio = '/home/mariano/Documentos/Simposio/kinoveatxt/norm/'
REF_NORM = map(
    lambda ruta: path.join(directorio, ruta),
    listdir(directorio)
    )
...
Matiak_cadera_POSTQX = np.asarray(Matiak_cadera_POSTQX).mean(axis=0)
Matiak_rodilla_POSTQX = np.asarray(Matiak_rodilla_POSTQX).mean(axis=0)
Matiak_tobillo_POSTQX = np.asarray(Matiak_tobillo_POSTQX).mean(axis=0)
