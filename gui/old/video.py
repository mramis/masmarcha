...
from core.video_ import VideoReader, VideoDrawings, VideoWriter
...
class VideoUtil(GridLayout):
    ...
    def record(self):
        ...
        rwqueue = queue.Queue(maxsize=1)
        rwevent = threading.Event()

        VideoReader(rwqueue, rwevent, self.config).start()
        VideoWriter(rwqueue, rwevent, self.config).start()
